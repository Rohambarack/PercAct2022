...
list_of_files <- list.files(path = "C:/github/PercAct2022/crossmodal/data_aud",
                            recursive = TRUE,
                            pattern = "\\.csv$",
                            full.names = TRUE)

DF1aud <- readr::read_csv(list_of_files, id = "file_name")


list_of_files <- list.files(path = "C:/github/PercAct2022/crossmodal/data_vis",
                            recursive = TRUE,
                            pattern = "\\.csv$",
                            full.names = TRUE)

DF2vis <- readr::read_csv(list_of_files, id = "file_name")
...
congruent dta = faster reaction time
...
myfazs<-function(x){
  ifelse(letsee$accuracy==TRUE,FALSE,TRUE)
}
...
#letsee2 <- letsee %>% 
 # mutate(across(.cols = ends_with("ccuracy"), ~myfazs(.)))

letsee2 <- letsee %>% 
mutate(across(.cols = ends_with("ccuracy"), ~ifelse(as.logical(.)==TRUE,FALSE,TRUE)))


DF1aud_2 <- merge(letsee2,DF1aud, by = "subject", all.x = TRUE)
